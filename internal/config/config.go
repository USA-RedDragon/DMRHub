...
type Config struct {
	...
	Redis        Redis    `name:"redis" description:"Redis configuration for the application"`
	Database     Database `name:"database" description:"Database configuration for the application"`
	Secret       string   `name:"secret" description:"Secret key for the application, used for signing and encryption of the user session"`
	PasswordSalt string   `name:"password-salt" description:"Salt used for hashing user passwords, should be a random string of sufficient length"`
	HTTP         HTTP     `name:"http" description:"HTTP server configuration for the application"`
	DMR          DMR      `name:"dmr" description:"DMR server configuration for the application"`
	SMTP         SMTP     `name:"smtp" description:"SMTP configuration for sending emails"`
	NetworkName  string   `name:"network-name" description:"Name of the DMR network, used in various places like the network status page" default:"DMRHub"`
	Metrics      Metrics  `name:"metrics" description:"Metrics configuration for the application"`
	PProf        PProf    `name:"pprof" description:"PProf configuration for the application, used for profiling and debugging purposes"`
...
type Redis struct {
	Enabled  bool   `name:"enabled" description:"Enable Redis support" default:"false"`
	Host     string `name:"host" description:"Redis host address"`
...
type Database struct {
	Driver          DatabaseDriver `name:"driver" description:"Database driver to use" default:"sqlite"`
	Database        string         `name:"database" description:"Database name or path" default:"DMRHub.db"`
	Host            string         `name:"host" description:"Database host address"`
...
type HTTP struct {
	...
	TrustedProxies []string  `name:"trusted-proxies" description:"List of trusted proxy IPs for the HTTP server"`
	CanonicalHost  string    `name:"canonical-host" description:"Canonical host for the HTTP server, used for generating absolute URLs"`
}
...
type RobotsTXT struct {
	Mode    RobotsTXTMode `name:"mode" description:"Mode for serving robots.txt. One of allow, disabled, or custom" default:"disabled"`
	Content string        `name:"content" description:"Content of the robots.txt file"`
}

// DMR holds the DMR server configuration.
type DMR struct {
	HBRP       HBRP       `name:"hbrp" description:"HBRP server configuration for DMR"`
	OpenBridge OpenBridge `name:"openbridge" description:"OpenBridge server configuration for DMR"`
}
...
type SMTP struct {
	Enabled    bool           `name:"enabled" description:"Enable SMTP support for sending emails" default:"false"`
	Host       string         `name:"host" description:"SMTP server host address"`
	Port       int            `name:"port" description:"SMTP server port"`
	TLS        SMTPTLS        `name:"tls" description:"SMTP TLS mode" default:"none"`
	Username   string         `name:"username" description:"SMTP server username"`
	Password   string         `name:"password" description:"SMTP server password"`
	From       string         `name:"from" description:"Email address to use as the sender"`
	AuthMethod SMTPAuthMethod `name:"auth-method" description:"SMTP authentication method. One of none, plain, or login" default:"none"`
}
